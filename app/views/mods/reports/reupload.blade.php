...
<table width="100%" cellspacing="0" cellpadding="0" class="doublecelltable">
	<tbody>
		<tr>
			<td width="100%">
				@if(count($reports) == 0)
					<center><h2>No links awaiting review</h2></center>
				@else
				<h1>Links need to be re uploaded</h1>
				...
				<table cellpadding="0" cellspacing="0" class="data">
					<tbody>
						<tr class="firstr">
							...
							<th class="width50perc center">link</th>
						...
						<tr class="@if($counter % 2 == 0)even @else odd @endif" id="torrent_{{$r->id}}">
							...
							<td>
									<a href="{{ $r->get_link()->link }}" target="_blank"> {{ $r->get_link()->link }} </a>
							</td>
							<td class="center"> {{ $r->get_owner() }} </td>				
							<td class="center"> 
								<a onclick="requestReupload('37', this);" class="icon16 textButton width100perc irefresh"><span></span>Accept</a><br><br>
